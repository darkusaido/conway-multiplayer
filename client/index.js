...
function domReady() {
    ...
    var generationNumberText = document.getElementById("generation-number");
    if (!tds) {
        throw ("table cells not found");
    }
    if (!runButton) {
        throw ("run button not found");
    }
    if (!stopButton) {
        throw ("stop button not found");
    }
    if (!clearButton) {
        throw ("clear button not found");
    }
    if (!runningText) {
        throw ("running text not found");
    }
    if (!generationNumberText) {
        throw ("generation number not found");
    }
    ...
    var clearAllCells = function clearAllCellsHandler() {
        var liveOnes = document.getElementsByClassName("live");
        for (var i = 0; i < liveOnes.length; i++) {
            var liveOne = liveOnes[i];
    ...
    socket.on("join", function socketJoinHandler(liveCells, isRunning, generationNumber) {
        ...
        clearAllCells();
        for (var cellKey in liveCells) {
            var cell = document.getElementById(cellKey);
            if (!cell) {
                return;
            }
            if (!cell.classList.contains("live")) {
